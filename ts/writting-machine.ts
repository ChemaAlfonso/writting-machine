...
class WritterMachine {
    ...
    constructor( writterElement: HTMLElement, { texts = [''], speed = 250, prefix = '', sufix = '', enableStyles = true, customStyles = '', lowerLimit = 0, upperLimit = 0 } ) {
        if( writterElement ){
            if( enableStyles )
    ...
    private makeWritter( writterElement: HTMLElement, texts: string[], speed: number, prefix: string, sufix: string, lowerLimit: number, upperLimit: number ) {
    ...
    stopWritter(): void {
     ...
     private setStyles( writterElement: HTMLElement, customStyles: string, speed: number ): void {
